#!/usr/bin/env python3
"""
Ops-compatible PayTM shim: merge PAYTM overlays, optional deferred anchors (internal API
polling), then run a thin TP_PAYTM_* script via runpy.

Canonical in monorepo: integrations/b_auto_bot/tp_127_executabes/run_paytm_bot.py
(or copy into any b_auto checkout: tp_127_executabes/run_paytm_bot.py).
Working directory must be the bot repo root (worker sets cwd / PYTHONPATH).

Input hook matches prompts in tp_127_bank_mains/tp_127_paytm*_main.py:
  - Login pause: "login maully" (main, trj) or "login manually" (new) — OTP is only in-browser.
  - Anchors: "Enter last CUSTOMER NAME:" / "Enter last TRANSACTION ID:" where present.
  - tp_127_paytm_trj_main.py has no customer-name input(); use --mode txn for TRJ cores.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Ops PayTM bridge for b_auto")
    ap.add_argument("--mode", choices=("txn", "name"), required=True)
    ap.add_argument("--profile", required=True, help="PAYTM[profile] dict key")
    ap.add_argument(
        "--thin-script",
        required=True,
        help="Relative path inside b_auto repo, e.g. tp_127_executabes/TP_PAYTM_TXN_MID.py",
    )
    ap.add_argument(
        "--defer-anchors",
        action="store_true",
    ...
    ap.add_argument(
        "--config-file",
        help="Merchant JSON overlay (already merged by worker snapshot).",
    )
    ns = ap.parse_args()
