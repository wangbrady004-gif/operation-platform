...
import { createMerchant, type MerchantFormResult } from './actions';
...
const BANKS: BankType[] = [
  // UPI / Wallets
  { id: 'google',    label: 'Google Pay',      group: 'UPI / Wallets',        profilePrefix: 'GOOGLE_',       loginType: 'gmail_pass',     domain: 'pay.google.com' },
  { id: 'paytm',    label: 'Paytm',           group: 'UPI / Wallets',        profilePrefix: 'PAYTM_',        loginType: 'paytm_login',    domain: 'paytm.com' },
...
export function CreateMerchantForm() {
  const [state, action] = useActionState(createMerchant, null as MerchantFormResult | null);
  ...
  useEffect(() => {
    if (!state) return;
    if (state.ok) {
      toast.success('Profile saved', { id: 'merchant-create', description: 'Visible in Run Session.' });
      setBank(null);
      setProfileKey('');
      return;
    }
    toast.error('Could not save', { id: 'merchant-create-err', description: state.message });
  }, [state]);
    ...
    <section className="rounded-xl border border-zinc-800 bg-zinc-900">
      ...
      <div className="p-5">
        <form action={action} className="flex flex-col gap-4">
          ...
          {/* Primary credential — hidden for paytm_login */}
          {showCreds && credLabel && (
            <div>
          ...
          {/* Password — hidden for paytm_login */}
          {showCreds && (
            <div>
