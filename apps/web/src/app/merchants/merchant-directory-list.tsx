...
import { MerchantDetailAccordions } from './merchant-detail-accordions';
...
function MerchantRegistryRow({
  row,
  defaultExpanded,
  defaultOpenEdit,
}: {
  row: MerchantDirectoryRow;
  defaultExpanded: boolean;
  defaultOpenEdit: boolean;
}) {
  const rootRef = useRef<HTMLDetailsElement>(null);
  const [expanded, setExpanded] = useState(defaultExpanded);
  const [detail, setDetail] = useState<MerchantDetailPlain | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/merchants/${row.id}`, { credentials: 'same-origin' });
        const body = (await res.json()) as MerchantDetailPlain & { error?: string };
        if (!res.ok) {
          throw new Error(body.error ?? res.statusText ?? 'Failed to load');
        }
        if (!cancelled) setDetail(body as MerchantDetailPlain);
      } catch (e) {
        if (!cancelled) {
  ...
  async function refetchDetail() {
    ...
    try {
      ...
      setDetail(body as MerchantDetailPlain);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to load profile');
    } finally {
      setLoading(false);
      ...
      >
        <summary className={summaryClasses}>
          ...
          <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
            <MerchantAdminActions id={row.id} profileKey={row.profileKey} showViewLink={false} showEditLink={false} />
          </div>
        </summary>
...
export function MerchantDirectoryList({
  rows,
  initialOpenMerchantId,
  initialOpenEdit,
}: {
  rows: MerchantDirectoryRow[];
  /** Deep-link: expand this profile (`?open=<uuid>`). */
  initialOpenMerchantId?: string | null;
  /** With `initialOpenMerchantId`, open the Edit accordion (`?edit=1`). */
  ...
  const openId = initialOpenMerchantId ?? null;
