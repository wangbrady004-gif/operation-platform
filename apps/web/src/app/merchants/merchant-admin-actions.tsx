...
import { MerchantGenerateModal } from './merchant-generate-modal';
...
export function MerchantAdminActions({
  ...
  /** Override default registry deep link `/merchants?open=:id&edit=1`. */
  editHref?: string;
}) {
  ...
  function onDelete() {
    if (
      !confirm(
        `Delete merchant «${profileKey}»? This cannot be undone. Jobs in history may still mention this merchant id.`,
      )
    ) {
      return;
    }
    start(async () => {
      ...
      if (r.ok) {
        ...
        toast.error('Could not delete merchant', {
          id: `delete-merchant-error-${id}`,
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {showEditLink ? (
          <Link
            href={editHref ?? `/merchants?open=${encodeURIComponent(id)}&edit=1`}
        ...
        {showViewLink ? (
          <Link
            href={`/merchants?open=${encodeURIComponent(id)}`}
        ...
        <button
          type="button"
          disabled={busy}
          aria-label={busy ? 'Deleting merchant' : 'Delete merchant'}
