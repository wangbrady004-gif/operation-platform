...
import { BankDirectoryList, type BankDirectoryRow } from './bank-directory-list';
...
async function fetchBanksAdmin(): Promise<BankDirectoryRow[]> {
  const auth = await getBearerHeaders();
  if (!auth) return [];
  const base = getOpsApiBaseUrl();
  const res = await fetch(`${base}/banks/admin`, { headers: auth, cache: 'no-store' });
  if (!res.ok) return [];
  return (await res.json()) as BankDirectoryRow[];
}
...
export default async function BanksAdminPage({
  searchParams,
}: {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
          ...
          <div className="space-y-4">
            ...
            <BankDirectoryList
              rows={rows}
              initialOpenBankId={initialOpenBankId}
              initialOpenEdit={initialOpenBankId ? initialOpenEdit : false}
            />
          </div>
